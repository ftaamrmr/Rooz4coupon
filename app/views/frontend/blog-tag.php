...
<div class="container mx-auto px-4 py-8">
    <?= breadcrumbs([
        ...
        __('Tag', 'الوسم') . ': ' . getLocalizedField($tag, 'name') => ''
    ]) ?>
    ...
    <div class="flex flex-col lg:flex-row gap-8">
        ...
        <div class="flex-1">
            <div class="mb-6">
                <span class="text-gray-500"><?= __('Tag', 'الوسم') ?>:</span>
                <h1 class="text-3xl font-bold inline-block ml-2 bg-indigo-100 dark:bg-indigo-900 text-indigo-700 dark:text-indigo-200 px-4 py-2 rounded-lg">
                    #<?= e(getLocalizedField($tag, 'name')) ?>
                </h1>
            </div>
            
            <p class="text-gray-600 dark:text-gray-400 mb-8">
                <?= sprintf(__('%d articles tagged with "%s"', '%d مقال موسوم بـ "%s"'), $totalArticles, getLocalizedField($tag, 'name')) ?>
            </p>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl p-12 text-center">
                ...
                <h3 class="text-lg font-medium text-gray-600 dark:text-gray-400">
                    <?= __('No articles with this tag yet', 'لا توجد مقالات بهذا الوسم بعد') ?>
                </h3>
            </div>
            <?php else: ?>
            ...
            <?= paginate($currentPage, $totalPages, url('blog/tag/' . $tag['slug'])) ?>
